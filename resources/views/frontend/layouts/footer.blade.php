<footer class="bottom dark">
    <div class="container" >
        <div class="row">
            <div class="col-lg-3" style="padding-top:40px">
                <div class="item abot">
                    <div class="logo-footer mb-20">
                        <a href="https://dev.vgsgroup.net/" class="logo">
                            <img src="{{asset('frontend/images/PB_VGS_white.svg')}}" width="200px" height="76px" alt="">
                        </a>
                    </div>

{{--                    <div class="social-icon">--}}
{{--                        <a href=""><i class="fa fa-twitter"></i></a>--}}
{{--                        <a href=""><i class="fa fa-facebook"></i></a>--}}
{{--                        <a href=""><i class="fa fa-linkedin"></i></a>--}}
{{--                    </div>--}}
                </div>
            </div>

            <div class="col-lg-3" style="padding-top:40px">
                <div class="item fotcont">
                    <div class="fothead">
                        <h6>Điện Thoại</h6>
                    </div>
                    <p>{{ isset($contact['phone']) ? $contact['phone'] : "" }}</p>
                </div>
            </div>
            <div class="col-lg-3" style="padding-top:40px">
                <div class="item fotcont">
                    <div class="fothead">
                        <h6>Email</h6>
                        <span></span>
                    </div>
                    <p>{{ isset($contact['email']) ? $contact['email'] : "" }}</p>

                </div>
            </div>
            <div class="col-lg-3" style="padding-top:40px">
                <div class="item fotcont">
                    <div class="fothead">
                        <h6>Địa Chỉ</h6>
                    </div>
                    <p>{{ isset($contact['address']) ? $contact['address'] : "" }}</p>
                </div>
            </div>
        </div>
    </div>
    <hr style="color: gray">
    <div class="sub-footer">
        <div class="container">
            <div class="row">
                <div class="col-lg-8">
                    <div class="text-left">
                        <p>© 2022, VGS GROUP</p>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="text-right">
                        <p>All right reserved.</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</footer>
<div class='fixed-right'>
    <a href='{{ isset($contact['facebook']) ? $contact['facebook']: "" }}' target='_blank' title='Facebook'>
        <svg xmlns="http://www.w3.org/2000/svg" data-name="Ebene 1" viewBox="0 0 1024 1024">
            <path fill="#1877f2"
                  d="M1024,512C1024,229.23016,794.76978,0,512,0S0,229.23016,0,512c0,255.554,187.231,467.37012,432,505.77777V660H302V512H432V399.2C432,270.87982,508.43854,200,625.38922,200,681.40765,200,740,210,740,210V336H675.43713C611.83508,336,592,375.46667,592,415.95728V512H734L711.3,660H592v357.77777C836.769,979.37012,1024,767.554,1024,512Z"/>
            <path fill="#fff"
                  d="M711.3,660,734,512H592V415.95728C592,375.46667,611.83508,336,675.43713,336H740V210s-58.59235-10-114.61078-10C508.43854,200,432,270.87982,432,399.2V512H302V660H432v357.77777a517.39619,517.39619,0,0,0,160,0V660Z"/>
        </svg>

    </a>
    <a href='{{ isset($contact['zalo']) ? $contact['zalo']: "" }}' title='Zalo'>
        <img alt='icon'
             src='https://1.bp.blogspot.com/-dbDziPVZ6FM/XqD6-L5R6LI/AAAAAAAAIs8/zAruLZY1XmY8VBc4Q80_WtrkU5aix_NSgCLcBGAsYHQ/s1600/icon-menu-right2.png'/>
    </a>
    <a class='phone' href="tel:{{$contact['phone']}}" title='0901.803.688'>
        <img style="padding-bottom: 10px" alt='icon'
             src='https://1.bp.blogspot.com/-n_xallSAwIA/XqD7C611tlI/AAAAAAAAItA/zIlgBND8WAI2KqbIP3wd1cTgyqP3sfuAgCLcBGAsYHQ/s1600/icon-menu-right3.png'/>
    </a>
</div>